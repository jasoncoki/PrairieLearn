...
module.exports.checkInfoValid = function(idName, info, infoFile, courseInfo) {
    ...
    if (idName == 'ciid') {
        if (_(info).has('allowIssueReporting')) {
            return new Error(`${infoFile}: "allowIssueReporting" is not longer permitted in "infoCourseInstance.json". Instead, set "allowIssueReporting" in infoAssessment.json files.`);
        }
    }
        ...
        async.filter(files, function(dirName, callback) {
            ...
            async.each(folders, function(dir, callback) {
                ...
                jsonLoad.readInfoJSON(infoFile, schemaFilename, optionSchemaPrefix, optionSchemaSuffix, function(err, info) {
                    ...
                    info[idName] = dir;
                    info.directory = dir;
                    err = that.checkInfoValid(idName, info, infoFile, courseInfo);
                    if (ERR(err, callback)) return;
                    if (info.disabled) {
                        callback(null);
                        return;
